...
object WorldUtils {
    fun getStateAt(pos: BlockPos) = SoTerm.mc.level?.getBlockState(pos) ?: Blocks.AIR.defaultBlockState()
    ...
    fun setBlockAt(pos: BlockPos, state: BlockState) = SoTerm.mc.level?.setBlock(pos, state, 19)

    fun isChunkLoaded(x: Int, z: Int): Boolean {
        return SoTerm.mc.level?.getChunk(x shr 4, z shr 4, ChunkStatus.FULL, false) != null
    }

    fun getBlockEntityList(): List<BlockPos> {
        val player = SoTerm.mc.player ?: return emptyList()
        val level = SoTerm.mc.level ?: return emptyList()
        val renderDistance = SoTerm.mc.options.renderDistance().get()
