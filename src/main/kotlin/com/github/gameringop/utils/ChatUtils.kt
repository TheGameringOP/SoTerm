...
object ChatUtils {
    ...
    fun modMessage(msg: Any?) = chat("${SoTerm.PREFIX} $msg")

    fun debug(flag: String, msg: Any?) {
        if (SoTerm.debugFlags.contains(flag)) modMessage(msg)
    }
    ...
    fun clickableChat(message: String, prefix: Boolean = false, command: String? = null, hover: String? = null, copy: String? = null) {
        ...
        ChatUtils.chat(if (prefix) Component.literal(SoTerm.PREFIX + " ").append(mainComponent) else mainComponent)
    }
