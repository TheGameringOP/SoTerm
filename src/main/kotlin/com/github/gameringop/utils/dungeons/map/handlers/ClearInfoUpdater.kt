...
import com.github.gameringop.features.impl.dungeon.map.DungeonMap
...
object ClearInfoUpdater {
    ...
    private val cdebug get() = SoTerm.debugFlags.contains("clearinfo")
    ...
    fun initStartSecrets() = SoTerm.scope.launch(Dispatchers.IO) {
    ...
    fun sendClearInfoMessage() = SoTerm.scope.launch(Dispatchers.IO) {
        if (! DungeonMap.enabled) return@launch
        if (! MapConfig.printPlayersClearInfo.value) return@launch
        ...
        val msgList = teammates.map { teammate ->
            ...
            val baseComp = createComponent("${SoTerm.PREFIX} $playerFormatted&f:&r ")
