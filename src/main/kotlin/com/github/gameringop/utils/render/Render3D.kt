...
object Render3D {
    ...
    fun renderBlock(
        ...
        if (outline) renderLineBox(
            ctx.matrixStack.last(),
            ctx.consumers.getBuffer(if (phase) OPRenderLayers.LINES_THROUGH_WALLS else OPRenderLayers.LINES),
            minX, minY, minZ, maxX, maxY, maxZ,
    ...
    fun renderCircle(
        ...
        ctx.matrixStack.pushPose()
        ctx.matrixStack.translate(ctx.camera.position().reverse())
        val buffer = ctx.consumers.getBuffer(if (phase) OPRenderLayers.CIRCLE_FILLED_THROUGH_WALLS else OPRenderLayers.CIRCLE_FILLED)
    ...
    fun renderBillboardedCircle(
        ...
        val layer = if (phase) OPRenderLayers.FILLED_THROUGH_WALLS else OPRenderLayers.FILLED
    ...
    fun renderLine(ctx: RenderContext, start: Vec3, finish: Vec3, color: Color, thickness: Number = 2, phase: Boolean = false) {
        ...
        val lines = if (phase) OPRenderLayers.LINES_THROUGH_WALLS else OPRenderLayers.LINES
    ...
    fun renderTracer(ctx: RenderContext, point: Vec3, color: Color, thickness: Number = 2.5) {
        ...
        val buffer = (ctx.consumers as MultiBufferSource.BufferSource).getBuffer(OPRenderLayers.LINES_THROUGH_WALLS)
        ...
        ctx.consumers.endBatch(OPRenderLayers.LINES_THROUGH_WALLS)
        ctx.matrixStack.popPose()
    }
