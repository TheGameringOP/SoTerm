...
object FarmKeys: Feature("Farm Keys") {
    ...
    private val sprintWithForward by ToggleSetting("Sprint with Forward", false).showIf { autoDirectionToggle.value }
    ...
    override fun init() {
        register<WorldChangeEvent> {
            if (active && LocationUtils.world != WorldType.Garden) {
                disableModule()
                if (SoTerm.debugFlags.contains("farm")) {
                    ChatUtils.modMessage("§cFarm Keys disabled because you left the Garden!")
                }
            }
        }
        ...
        register<KeyboardEvent.KeyPressed> {
            if (!active || !autoDirectionToggle.value || LocationUtils.world != WorldType.Garden) return@register
            ...
            when {
                ...
                key == alwaysForwardKey.value -> {
                    ...
                    if (autoMoveForward && sprintWithForward.value) mc.options.keySprint.setDown(true)
                    event.isCanceled = true
                }
        ...
        register<TickEvent.Start> {
            if (mc.player == null || !active) return@register
            ...
            if (autoMoveForward) {
                ...
                if (sprintWithForward.value) mc.options.keySprint.setDown(true)
            }
    ...
    private fun enableModule() {
        ...
        if (SoTerm.debugFlags.contains("farm")) ChatUtils.modMessage("§eFarm mode: §aON")
    }
    ...
    private fun disableModule() {
        ...
        mc.options.save()
        KeyMapping.resetMapping()
        if (SoTerm.debugFlags.contains("farm")) ChatUtils.modMessage("§eFarm mode: §cOFF")
    }
