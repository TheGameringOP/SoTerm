...
object TerminalSolver: Feature("Renders solutions for Floor 7 terminals.") {
    ...
    val showNoSafeButton = ToggleSetting("Show NoSafe Button").showIf { melodyBlock.value }
        ...
        when (type) {
            ...
            TerminalType.COLORS -> {
                ...
                fun fixName(name: String): String {
                    var fixedName = name
                    TerminalType.colorReplacements.forEach { (k, v) -> fixedName = fixedName.replace(k, v) }
                    return fixedName
                }
                currentItems.filter {
    ...
    fun onTerminalOpen() {
        isClicked = false
        TerminalType.reset()
        TerminalType.melodyState.correct = null
        TerminalType.melodyState.button = null
